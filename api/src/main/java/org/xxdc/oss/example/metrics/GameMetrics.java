...
public class GameMetrics {
  ...
  static Gatherer<Map<Integer, List<Integer>>, GameState, GameMetricWinningPath> winningPath = null; 
  ...
  private Set<List<Integer>> findNewWinningPaths(GameBoard board, int position, int playerValue) {
    ...
    var rowPath = IntStream.range(0, dimension).map(col -> row * dimension + col).boxed().toList();
    paths.add(rowPath);

    // Check vertical path
    var col = position % dimension;
    var colPath = IntStream.range(0, dimension).map(r -> r * dimension + col).boxed().toList();
    paths.add(colPath);

    // Check diagonals if position is on them
    if (row == col) {
      var diagPath = IntStream.range(0, dimension).map(i -> i * dimension + i).boxed().toList();
      paths.add(diagPath);
    }
