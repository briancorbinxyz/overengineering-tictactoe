...
public abstract sealed class SecureMessageHandler implements MessageHandler {
	...
	/**
	 * Initializes the SecureMessageHandler implementation. This method must be called before any other methods can be used.
	 * 
	...
	/**
	 * Exchange a shared secret key with remote connection using the Kyber Key Encapsulation Mechanism (KEM).
	 *
	 * This method generates a new Kyber key pair, publishes the public key to the remote handler, and then receives an
	 * encapsulated shared secret from the remote handler. It uses the private key to decapsulate the shared secret.
	 *
	 * @return the shared secret key
	 * @throws NoSuchAlgorithmException 
	 * @throws IllegalArgumentException NoSuchAlgorithmException if the "Kyber" algorithm is not available
	...
	public void sendMessage(String message) throws IOException {
		...
		try {
			...
			//System.out.println("SENT ENCRYPTED: " + Base64.getEncoder().encodeToString(ivAndCiphertext));
	...
	/**
	 * Receives an encrypted message from the remote handler, decrypts it using the shared secret key, and returns the decrypted message.
	*
	* This method first receives the encrypted message and the initialization vector (IV) from the remote handler. It then
	* extracts the ciphertext from the received data, initializes an AES-CBC cipher with the shared secret key and the IV,
	* and decrypts the ciphertext. Finally, it returns the decrypted message as a String.
	*
	* @return the decrypted message
	* @throws IOException if there is an error receiving the message from the remote handler
	* @throws IllegalArgumentException if there is an error with the security configuration or exchange
	*/
	@Override
	public String receiveMessage() throws IOException {
		...
		try {
			...
			//System.out.println("RECEIVED ENCRYPTED: " + Base64.getEncoder().encodeToString(ivAndCiphertext));
			var cipher = newCipherInstance();
	...
	* This class extends the `SecureMessageHandler` class and is responsible for initializing the secure channel,
	* exchanging the shared secret key with the client, and providing methods for sending and receiving encrypted messages.
	*/
	public static final class Server extends SecureMessageHandler {
		...
		*
		* @param handler the `RemoteMessageHandler` to use for sending and receiving messages
		*/
		public Server(RemoteMessageHandler handler) {
			super(handler);
		}
	
		/**
		 * Initializes the secure message handler by setting up the secure channel and exchanging the shared secret key with the client.
		*
		* @throws IOException if there is an error during the initialization process
		*/
		@Override
		public void init() throws IOException {
			try {
				...
				initialized = true;
				System.out.println("Secure connection for " + getClass().getSimpleName() + " established with " + sharedKey.getAlgorithm() + " shared key.");
		...
		/**
		 * Exchanges the shared secret key with the client using the Kyber key encapsulation mechanism (KEM).
		*
		* @return the shared secret key
		...
		*/
		protected SecretKey exchangeSharedKey()
				throws NoSuchAlgorithmException, IOException, NoSuchProviderException, InvalidParameterSpecException,
				InvalidAlgorithmParameterException, InvalidKeyException, DecapsulateException {
	...
	* This class extends the `SecureMessageHandler` class and is responsible for initializing the secure channel,
	* exchanging the shared secret key with the server, and providing methods for sending and receiving encrypted messages.
	*/
		...
		*/
		...
		public void init() throws IOException {
			...
			try {
				...
				initialized = true;
				System.out.println("Secure connection for " + getClass().getSimpleName() + " established with " + sharedKey.getAlgorithm() + " shared key.");
			} catch (ClassNotFoundException | IOException | NoSuchAlgorithmException | NoSuchProviderException | InvalidKeyException | InvalidAlgorithmParameterException e) {
				throw new IllegalArgumentException("Invalid security configuration/exchange: " + e.getMessage(), e);
			}
		}
		...
		/**
			...
			*/
		protected SecretKey exchangeSharedKey() throws NoSuchAlgorithmException, NoSuchProviderException, ClassNotFoundException,
		 ...
		 * @throws ClassNotFoundException if the class is not found whilst deserializing from the message handler
